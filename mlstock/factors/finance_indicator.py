...
class FinanceIndicator(FinanceFactor, FillMixin, TTMMixin):
    ...
    FIELDS_DEF = [
        ...
        I('tangible_assets_to_total_liability', '有形资产比负债合计', '长期偿债能力'),
        # ('ebit_to_interest','已获利息倍数(EBIT比利息费用)'),
        # 盈利能力
        I('total_profit_to_operate', '利润总额比营业收入', ' 盈利能力'),
        ...
        # 财务质量
        I('', 'profit_dedt', '扣除非经常性损益后的净利润（扣非净利润）', '财务质量', ttm=True, normalize=True),
        # 华泰金工要求的是"扣除非经常性损益后净利润（TTM）/总市值"
        I('', 'ocfps', '每股经营活动产生的现金流量净额', '财务质量', ttm=True, normalize=True),  # 华泰金工要求的是泰金工上的是"经营性现金流（TTM）/总市值"，用这个代替
        I('ROE_TTM', 'ROE_TTM', '净资产收益率', '财务质量', ttm=True, normalize=True),  # ROE（最新财报，TTM）
        # I('','ROA_yoy', 这个tushare上没有
        I('ROA_TTM', 'ROA_TTM', '总资产报酬率', '财务质量', ttm=True, normalize=True)  # ROA（最新财报，TTM）
